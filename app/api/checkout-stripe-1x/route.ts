...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const session = await stripe.checkout.sessions.create({
      mode: 'payment',
      // Carte uniquement (paiement en 1 fois) — contrairement à /api/checkout-stripe
      // qui laisse Stripe proposer Scalapay et les autres méthodes activées.
    ...
    if (process.env.SUPABASE_SERVICE_ROLE_KEY) {
      await supabase.from('orders').insert({
        ...
        if (error) console.error('[checkout-stripe-1x] order save error:', error.message)
      })
    }

    return NextResponse.json({ url: session.url, reference })
  } catch (err: any) {
    console.error('[checkout-stripe-1x]', err.message)
